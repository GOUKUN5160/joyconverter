...
const watchTargets = [
  selectedType,
  selectedKeyboardActionType,
  selectedKeyboardAction,
  selectedMouseAction,
  selectedMouseClickMode,
  selectedOtherAction,
...
const element2Data = () => {
  ...
  if (selectedType.value == "keyboard") {
    args = JSON.stringify({
      action: selectedKeyboardActionType.value,
      value: selectedKeyboardAction.value,
    });
    ...
    if (
      ...
      args = JSON.stringify({
        action: selectedMouseAction.value,
        value: selectedMouseClickMode.value,
      });
      ...
      args = JSON.stringify({
        action: "moveAbsolute",
        value: absoluteMove.value,
      });
    } else if (selectedMouseAction.value == "moveRelative") {
      args = JSON.stringify({
        action: "moveRelative",
        value: relativeMove.value,
      });
    }
    ...
    if (selectedOtherAction.value == "sleep") {
      ...
      args = JSON.stringify({
        ...
        value: selectedOtherProfile.value,
      });
...
const onKeyUp = (keyName: string, keyId: string) => {
  if (KeyInputFocused.value == "keyboard") {
    selectedKeyboardAction.value.push({
      name: keyName.toUpperCase(),
      id: keyId,
    });
  } else if (
    KeyInputFocused.value == "moveRelative" &&
    keyName.toLowerCase() == startStopKey.value.toLowerCase()
  ) {
...
const onMouseClick = (
  ...
  if (mouseInputFocused.value == "moveAbsolute") {
    ...
    temporaryClickData.value.push({
      id: (temporaryClickData.value.length + 1).toString(),
      button: buttonName,
      event: pressed ? "プレス" : "リリース",
      x: x.toString(),
      y: y.toString(),
    });
  }
...
<template>
  <v-row>
    ...
    <template v-if="selectedType == 'keyboard'">
      <v-col cols="2">
        <v-select
          v-model="selectedKeyboardActionType"
          label="アクション"
          density="compact"
          :item-props="itemProps"
      ...
      <v-col cols="7">
        <v-select
          v-model="selectedKeyboardAction"
          :item-props="itemProps"
          density="compact"
          variant="outlined"
          readonly
          ...
          clearable
          class="noicon-select"
          placeholder="順番にキーを押してください（1つずつ入力してください）"
          @update:focused="(isFocused: boolean) => { if (isFocused) { KeyInputFocused = 'keyboard'; } else { KeyInputFocused = ''; } setKeyInputFocused(isFocused); }"
        ></v-select>
      </v-col>
    </template>
    <template v-if="selectedType == 'mouse'">
      ...
      >
        <v-col cols="6">
          <v-select
            v-model="selectedMouseClickMode"
            label="モード"
            density="compact"
            :item-props="itemProps"
            :items="MOUSE_CLICK_MODE"
          ></v-select>
        </v-col>
      </template>
      <template v-if="selectedMouseAction == 'scroll'">
        <v-col cols="1.5">
          <Input
            before-text="上"
            max-width="100%"
            v-model="scrollValues.up"
            required="0"
            :number-min-max="`${SCROLL_MINMAX.min},${SCROLL_MINMAX.max}`"
            @update:model-value="scrollValues.down = '0'"
          ></Input>
        </v-col>
        <v-col cols="1.5">
          <Input
            before-text="下"
            max-width="100%"
            v-model="scrollValues.down"
            required="0"
            :number-min-max="`${SCROLL_MINMAX.min},${SCROLL_MINMAX.max}`"
            @update:model-value="scrollValues.up = '0'"
          ></Input>
        </v-col>
        <v-col cols="1.5">
          <Input
            before-text="左"
            max-width="100%"
            v-model="scrollValues.left"
            required="0"
            :number-min-max="`${SCROLL_MINMAX.min},${SCROLL_MINMAX.max}`"
            @update:model-value="scrollValues.right = '0'"
          ></Input>
        </v-col>
        <v-col cols="1.5">
          <Input
            before-text="右"
            max-width="100%"
            v-model="scrollValues.right"
            required="0"
            :number-min-max="`${SCROLL_MINMAX.min},${SCROLL_MINMAX.max}`"
            @update:model-value="scrollValues.left = '0'"
          ></Input>
        </v-col>
      </template>
      <template v-if="selectedMouseAction == 'moveAbsolute'">
        <v-col cols="2">
          <Input
            before-text="X"
            max-width="100%"
            v-model="absoluteMove.x"
            required="0"
            :readonly="mouseInputFocused == 'moveAbsolute'"
          ></Input>
        </v-col>
        <v-col cols="2">
          <Input
        ...
        <v-col cols="2">
          <v-btn
            @click="
              mouseInputFocused = 'moveAbsolute';
              setMouseMoveFocused(true);
              setMouseClickFocused(true);
            "
            :text="
              mouseInputFocused == 'moveAbsolute' ? 'クリックして確定' : '取得'
            "
            :disabled="mouseInputFocused == 'moveAbsolute'"
          ></v-btn>
        </v-col>
      </template>
            ...
            <template v-slot:activator="{ props: activatorProps }">
              <v-btn
                ...
                @click="
                  ...
                  temporaryCalcedData = { x: '', y: '' };
                  KeyInputFocused = 'moveRelative';
                  setMouseClickFocused(true);
                  setKeyInputFocused(true);
                "
                :text="
                  mouseInputFocused == 'moveRelative' ? '測定中' : '距離を測定'
                "
                :disabled="mouseInputFocused == 'moveRelative'"
              ></v-btn>
            </template>
            <template v-slot:default="{ isActive }">
              <v-card
                :title="
                  mouseInputFocused == 'moveRelative'
                    ? '測定中'
                    : '相対座標を計算'
                "
              >
                <v-card-text>
                  ...
                  <v-row justify="space-around">
                    ...
                    <v-col>
                      <v-select
                        ...
                        "
                        density="compact"
                        :items="START_STOP_KEY.map((key: string) => key.toUpperCase())"
                      ></v-select>
                    </v-col>
                  </v-row>
                  結果:
                  <template
                    v-if="
                      temporaryCalcedData.x != '' && temporaryCalcedData.y != ''
                    "
                    >({{ temporaryCalcedData.x }},
                    {{ temporaryCalcedData.y }})</template
                  >
                  <v-data-table-virtual
                    id="pointTable"
                    :items="temporaryClickData"
                    hide-default-header
                    hide-default-footer
                    height="400"
                    no-data-text="記録を開始してからポイントをクリックしてください"
                  >
                    <template v-slot:item.id="{ item }">
                      <v-selection-control-group
                        v-model="temporarySelectedData"
                      ...
                      >
                        <v-checkbox-btn
                          :value="{ id: item.id, position: [item.x, item.y] }"
                          :readonly="
                            Object.keys(temporarySelectedData).length >= 2 &&
                            !temporarySelectedData
                              .map((d) => d.id)
                              .includes(item.id)
                          "
                    ...
                    <v-col cols="auto">
                      ...
                      <v-btn
                        ...
                        @click="
                          ...
                          setKeyInputFocused(false);
                          isActive.value = false;
                          relativeMove = temporaryCalcedData;
                          element2Data();
                        "
                        :disabled="
                          temporaryCalcedData.x == '' ||
                          temporaryCalcedData.y == ''
                        "
                      ></v-btn>
                    </v-col>
                  </v-row>
    ...
    <template v-if="selectedType == 'other'">
      ...
      <v-col cols="5" v-if="selectedOtherAction == 'sleep'">
        <Input
          before-text="時間"
          :after-text="`ms （${Number(sleepTime) / 1000}秒）`"
          v-model="sleepTime"
          required="0"
          :number-min-max="`${SLEEP_MINMAX.min},${SLEEP_MINMAX.max}`"
          max-width="50%"
        ></Input>
      </v-col>
      <v-col cols="5" v-if="selectedOtherAction == 'changeProfile'">
        <v-select
          v-model="selectedOtherProfile"
          label="プロファイル"
          density="compact"
          :item-props="itemProps"
          :items="props.otherProfiles"
          no-data-text="他のプロファイルがありません"
        ></v-select>
      </v-col>
      ...
      <v-col cols="5" v-if="selectedOtherAction == 'rumble'">
        <Input
          before-text="振動時間"
          :after-text="`ms （${Number(rumbleTime) / 1000}秒）`"
          v-model="rumbleTime"
          required="100"
          :number-min-max="`${RUMBLE_MINMAX.min},${RUMBLE_MINMAX.max}`"
          max-width="50%"
        ></Input>
      </v-col>
