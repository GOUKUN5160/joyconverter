...
const onUpdatedJoyconList = (
  joyconList: { [key: string]: { [key: string]: any } }[]
) => {
  joycons.value = joyconList;
  const newSerials = joycons.value.map((joycon) => {
    return joycon.serial;
  });
  if (
    selectedJoyCon.value.serial &&
    newSerials.includes(selectedJoyCon.value.serial)
  ) {
    selectedJoyCon.value = joycons.value.filter((joycon) => {
      return joycon.serial == selectedJoyCon.value.serial;
    })[0];
  } else {
    selectedJoyCon.value = {};
  }
...
<template>
  ...
  >
    <v-list-item
      title="JoyCon一覧"
      :subtitle="`利用可能: ${
        joycons.filter((joycon) => {
          return joycon.is_calibrated && joycon.is_connected;
        }).length
      }`"
    ></v-list-item>
    <v-divider></v-divider>
    <div v-for="item in joycons" :key="item.serial">
      ...
      >
        <template v-slot:prepend v-if="item.is_connected">
          ...
          >
            <v-icon
              :icon="item.is_calibrated ? 'mdi-play' : 'mdi-pause'"
              :color="item.is_calibrated ? 'success' : 'red'"
            ></v-icon>
          </div>
        </template>
        <v-list-item-title
          :style="item.is_connected ? '' : 'color: darkgray;'"
          >{{ item.name }}</v-list-item-title
        >
        <template v-slot:append>
          <div
            :class="`joycon-list-wrapper joycon-list-${item.type.toLowerCase()}${
              item.is_connected ? '' : ' disabled'
            }`"
          >
            <JoyConImage
              :serial="item.serial"
              ...
              :status="{}"
              :body-color="item.color_body"
              :button-color="item.color_btn"
            ></JoyConImage>
          </div>
        </template>
      </v-list-item>
