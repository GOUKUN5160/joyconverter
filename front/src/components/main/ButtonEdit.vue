...
watch(
  basicOnPress,
  () => {
    console.log("press", basicOnPress.value);
  },
  { deep: true }
);
...
const watchTargets = [
  ...
  basicOnKeep,
  basicOnKeepRapid,
  basicOnKeepRapidInterval,
  basicOnKeepRapidDelay,
  basicOnKeepAnotherProfile,
  basicOnKeepAnotherProfileDelay,
  basicOnRelease,
  switchFunc,
  switchDelay,
  countFunc,
  ...
  if (selectedAction.value == "basic") {
    ...
    if (basicOnKeep.value == "rapid") {
      ...
      if (basicOnKeepAnotherProfile.value.name != undefined) {
        preview.push(
          `長押し=[他プロファイル (${basicOnKeepAnotherProfile.value.name})]`
        );
      }
      keep.value = {
    ...
    const isResetAnotherProfile = checkBoxes.value.includes(
      "switch-reset-anotherProfile"
    );
  ...
  <v-container fluid>
    ...
    <v-radio-group v-model="selectedAction">
      ...
      <Radio label="基本" value="basic" :modelValue="selectedAction">
        <Nest label="押したとき">
          <AddFunctions
            v-model="basicOnPress"
            :other-profiles="props.otherProfiles"
          ></AddFunctions>
        </Nest>
        <Nest label="押されているとき">
          <v-radio-group v-model="basicOnKeep">
            ...
            <Radio label="連射" value="rapid" :modelValue="basicOnKeep">
              ...
              <Input
                beforeText="間隔"
                afterText="ms"
                v-model="basicOnKeepRapidInterval"
                :number-min-max="basicOnkeepRapidMinMax"
                :required="defaultRapidInterval"
              ></Input>
              <Check
                label="一定時間経ってから"
                value="basic-onKeep-rapid-delay"
                v-model="checkBoxes"
              >
                <Input
                  beforeText="時間"
                  afterText="ms"
                  v-model="basicOnKeepRapidDelay"
                  :number-min-max="delayMinMax"
                  :required="defaultDelay"
                ></Input>
              </Check>
            </Radio>
            <Radio
              label="押されている間一時的に他のプロファイルを使う"
              value="another-profile"
              :modelValue="basicOnKeep"
            >
              <v-select
                v-model="basicOnKeepAnotherProfile"
                label="プロファイル"
                density="compact"
              ...
              <Check
                label="一定時間経ってから"
                value="basic-onKeep-anotherProfile-delay"
                v-model="checkBoxes"
              >
                <Input
                  beforeText="時間"
                  afterText="ms"
                  v-model="basicOnKeepAnotherProfileDelay"
                  :number-min-max="delayMinMax"
                  :required="defaultDelay"
                ></Input>
              </Check>
              <small style="display: block; padding-top: 8px"
                >※このボタンのプロファイルは変わりません</small
              >
            </Radio>
          </v-radio-group>
        </Nest>
        <Nest label="離したとき">
          <AddFunctions
            v-model="basicOnRelease"
            :other-profiles="props.otherProfiles"
          ></AddFunctions>
        </Nest>
      </Radio>
      <Radio
        label="ボタンが押されるたびに切り替える"
        value="switch"
      ...
      >
        ...
        <Check
          label="別プロファイルに移行したらリセット"
          value="switch-reset-anotherProfile"
          v-model="checkBoxes"
        >
          <small
            >※「一時的に他のプロファイルを使う」ではリセットされません</small
          >
        </Check>
      </Radio>
      <Radio
        label="連続して押された回数によって切り替える"
        value="count"
        :modelValue="selectedAction"
      >
        <AddFunctions
        ...
        <Input
          ...
          :number-min-max="countIntervalMinMax"
          :required="defaultCounInterval"
        ></Input>
      </Radio>
      <Radio
        label="ボタンが押される長さによって切り替える"
        value="length"
        :modelValue="selectedAction"
      >
        <Input
          ...
          afterText="ms"
          v-model="lengthTime"
          :number-min-max="lengthMinMax"
          :required="defaultLength"
        ></Input>
