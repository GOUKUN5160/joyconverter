...
const LED_PATTERN = ["0000", "1000", "0100", "1100", "0010", "1010", "0110", "1110", "0001", "1001", "0101", "1101", "0011", "1011", "0111", "1111"]
...
const allJoyCon = (joyconType: string) => {
  ...
  if (isAllJoyCon.value[joyconType]) {
    selectedJoyCon.value.unshift({ name: `全てのJoyCon(${joyconType.toUpperCase()})`, serial: `all-${joyconType}`, type: "none" });
  } else {
    selectedJoyCon.value = selectedJoyCon.value.filter((joycon: any) => joycon.serial != `all-${joyconType}`);
  }
};
...
watch([profiles, selectedJoyCon], () => {
  const useJoyCon = {
    l: {
      isAll: isAllJoyCon.value.l,
      list: selectedJoyCon.value.filter((joycon: any) => joycon.type.toLowerCase() == "l")
    },
    r: {
      isAll: isAllJoyCon.value.r,
      list: selectedJoyCon.value.filter((joycon: any) => joycon.type.toLowerCase() == "r")
    }
  };
  eel.set_convert_info(props.path, profiles.value, useJoyCon)().then((result: any) => {
...
watch(selectedProfile, () => {
  otherProfiles.value = profiles.value.filter((profile: any) => profile.value != selectedProfile.value);
  console.log("他のプロファイル", otherProfiles.value);
  selectedProfileIndex.value = profiles.value.findIndex((profile: any) => profile.value == selectedProfile.value);
  selectedLedPattern.value = LED_PATTERN[profiles.value[selectedProfileIndex.value].led];
...
const convertUseJoyConData = (data: any) => {
  ...
  const availableLeft = data.l.list.filter((joycon: any) => {
    const availableSerials = availableJoyCons.value.map((joycon: any) => joycon.serial);
  ...
  selectedJoyCon.value = selectedJoyCon.value.concat(availableLeft, availableRight).map((joycon: any) => {
...
const createProfile = () => {
  ...
  profiles.value.push({ name: "プロファイル" + (profiles.value.length + 1), value: randstr(10), main: false, led: 1, convert: [] });
...
const deleteProfile = () => {
  ...
  confirmDialogMessage.value = `${profiles.value[selectedProfileIndex.value].name}を削除しますか？設定を復元することはできません。`;
  confirmDialog.value = true;
  onDialogResponse = (index: number) => {
    if (index == 1) {
      profiles.value.splice(selectedProfileIndex.value, 1);
      selectedProfile.value = profiles.value[selectedProfileIndex.value - 1].value;
...
const exportProfile = () => {
  console.log("エクスポート");
  eel.export_profile(profiles.value[selectedProfileIndex.value])().then((result: any) => {
...
const init = () => {
  ...
  loading.value = true;
  eel.get_convert_info(props.path)().then((data: any) => {
    const convert = data[0];
    const useJoyCon = data[1];
    if (!convert) {
      snackbarErrorMessage.value = "プロファイルの取得に失敗しました。アプリを削除してやり直してください。";
      snackbarError.value = true;
      loading.value = false;
      return;
    }
    ...
    if (convert.length <= 0) {
      profiles.value = [{ name: "プロファイル1", value: randstr(10), main: true, led: 1, convert: [] }];
...
<template>
  ...
  <template v-if="props.path != ''">
    <v-select v-model="selectedJoyCon" @update:focused="(val: boolean) => { if (val) updateAvailableJoyCon(); }"
      ...
      <template v-slot:chip="{ item }">
        <v-chip v-if="!isAllJoyCon[Object(item.raw).type.toLowerCase()]"
          :color="Object(item.raw).type.toLowerCase() == 'l' ? 'blue' : Object(item.raw).type.toLowerCase() == 'r' ? 'red' : undefined">
          {{ item.title }}
        </v-chip>
      </template>
      ...
      <template v-slot:item="{ props, item }">
        <v-list-item v-bind="props" :disabled="isAllJoyCon[Object(item.raw).type.toLowerCase()]"
          :title="Object(item.raw).name" @click="console.log('これだぁ！', props, item, Object(item.raw))">
          <template v-slot:prepend>
            <v-checkbox-btn
              :color="Object(item.raw).type.toLowerCase() == 'l' ? 'blue' : Object(item.raw).type.toLowerCase() == 'r' ? 'red' : undefined"
    ...
    <EditList v-if="profiles[selectedProfileIndex] != undefined" v-model="profiles[selectedProfileIndex].convert"
      :other-profiles="otherProfiles"></EditList>
  ...
  <Dialog v-model="renameDialog" title="アプリ名の変更" icon="mdi-rename" :text="dialogMessage"
    ...
    <v-text-field v-model="profileName" :placeholder="defaultProfileName"
