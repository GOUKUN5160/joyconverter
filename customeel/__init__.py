...
if TYPE_CHECKING:
    from customeel.types import OptionsDictT, WebSocketT
else:
    WebSocketT = Any
    OptionsDictT = Any
...
import re as rgx
import os
import customeel.browsers as brw
...
_eel_js_file: str = pkg.resource_filename('customeel', 'eel.js')
...
_start_args: OptionsDictT = {
    ...
    'size':             (600, 800),                 # (width, height) of main window
    'position':         (0, 0),                     # (left, top) of main window
    ...
    'shutdown_delay': 20.0,                          # how long to wait after a websocket closes before detecting complete shutdown
    'class_instance':   None,                       # Allow a class instance (i.e. self) to be passed to exposed functions
    'app_name': 'Python CustomEel',                 # User agent string to use when starting the browser
    'title': 'Python CustomEel with Pywebview',     # Title of the webview window
}
...
def set_js_result_timeout(msec: int) -> None:
    global _js_result_timeout
    _js_result_timeout = msec
...
def show(*start_urls: str) -> None:
    global webview
    if _start_args['mode'] == 'webview':
        webview = brw.open(list(start_urls), _start_args)
    else:
...
def check_invalid_user_agent() -> bool:
    if _start_args['mode'] != 'webview':
        return True
    user_agent = btl.request.environ.get("HTTP_USER_AGENT")
    if user_agent == _start_args['app_name']:
        return True
    return False

def _eel() -> str:
    if not check_invalid_user_agent():
        return btl.abort(403, "Invalid Access")
...
def _get_real_path(path: str) -> str:
    if getattr(sys, 'frozen', False):
        ...
        return os.path.join(base_path, path) # type: ignore # sys._MEIPASS is dynamically added by PyInstaller
    else:
...
def _websocket_close(page: str) -> None:
    ...
    if close_callback is not None:
        ...
        if _start_args["mode"] == "webview":
            close_callback(webview, sockets, webview.geometry)
        else:
