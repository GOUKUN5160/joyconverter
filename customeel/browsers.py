...
import customeel.chrome as chm
import customeel.electron as ele
import customeel.edge as edge
from customeel.webviewmaster import GUI
...
def open(start_pages: Iterable[Union[str, Dict[str, str]]], options: OptionsDictT) -> None | GUI:
    ...
    if mode is None or mode is False:
        ...
        # Run with a webview window
        gui = GUI(options["title"], start_urls[0], options["app_name"], options["size"][0], options["size"][1], options["position"][0], options["position"][1])
